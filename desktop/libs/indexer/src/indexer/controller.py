...
from libzookeeper.conf import CLUSTERS
...
class CollectionManagerController(object):
  ...
  def create_collection(self, name, fields, unique_key_field='id', df='text'):
    ...
    if self.is_solr_cloud_mode():
      ...
      process = subprocess.Popen([solrctl_path, "--zk", CLUSTERS.get()['default'].ENSEMBLE.get(), "instancedir", "--create", name, solr_config_path],
      ...
      if not api.create_collection(name):
        ...
        process = subprocess.Popen([solrctl_path, "--zk", CLUSTERS.get()['default'].ENSEMBLE.get(), "instancedir", "--delete", name],
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
        if process.wait() != 0:
